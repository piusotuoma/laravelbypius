...
<html class="no-js" lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ in_array(app()->getLocale(), ['ar', 'az', 'dv', 'fa', 'he', 'ku', 'ur']) ? 'rtl' : 'ltr' }}">
	<head>
		...
		@if( config('app.debug') !== true )
			<meta http-equiv="Content-Security-Policy" content="base-uri 'self'; default-src 'self' 'nonce-{{ app( 'pius.context' )->get()->nonce() }}'; {{ config( 'shop.csp.frontend', 'style-src \'unsafe-inline\' \'self\'; img-src \'self\' data: https://piusenterprises.com; frame-src https://www.youtube.com https://player.vimeo.com' ) }}">
		@endif
		...
		@endif
		<link type="text/css" rel="stylesheet" href="{{ asset('vendor/shop/themes/default/pius.css?v=' . config( 'shop.version', 1 ) ) }}">

		@yield('pius_header')

		<style nonce="{{ app( 'pius.context' )->get()->nonce() }}">
			:root {
				@foreach( app( 'pius.context' )->get()->locale()->getSiteItem()->getConfigValue( 'theme/default', [] ) as $key => $value )
					{{ $key }}: {{ $value }};
				@endforeach
			}
		</style>

		<link rel="icon" href="{{ asset( 'pius/' . ( app( 'pius.context' )->get()->locale()->getSiteItem()->getIcon() ?: '../vendor/shop/themes/default/assets/icon.png' ) ) }}">
	...
	<body class="{{ $page ?? '' }}">
		<nav class="navbar navbar-expand-md navbar-top">
			<a class="navbar-brand" href="/" title="{{ __('To the home page') }}">
				<img src="{{ asset( 'pius/' . ( app( 'pius.context' )->get()->locale()->getSiteItem()->getLogo() ?: '../vendor/shop/themes/default/assets/logo.png' ) ) }}" height="40" alt="{{ __('To the home page') }}">
			</a>
			...
			@yield('pius_head_locale')
			@yield('pius_head_search')
				...
				@else
					<li class="nav-item login profile dropdown">
						...
						<ul class="dropdown-menu dropdown-menu-end" role="menu">
							<li class="dropdown-item"><a class="nav-link" href="{{ airoute( 'pius_shop_account' ) }}"><span class="name">{{ __( 'Profile' ) }}</span></a></li>
						...
						<div class="row">
							<div class="col-sm-6 footer-left">
								<div class="footer-block">
									...
									<p><a href="{{ airoute('pius_page', ['path' => 'terms']) }}">{{ __( 'Terms & Conditions' ) }}</a></p>
									<p><a href="{{ airoute('pius_page', ['path' => 'privacy']) }}">{{ __( 'Privacy Notice' ) }}</a></p>
									<p><a href="{{ airoute('pius_page', ['path' => 'cancel']) }}">{{ __( 'Cancellation' ) }}</a></p>
								</div>
							</div>
							<div class="col-sm-6 footer-center">
								<div class="footer-block">
									<h2 class="pb-3" aria-label="{{ __('About the company') }}">{{ __( 'ABOUT US' ) }}</h2>
									<p><a href="{{ airoute('pius_page', ['path' => 'contact']) }}">{{ __( 'Contact us' ) }}</a></p>
									<p><a href="{{ airoute('pius_page', ['path' => 'about']) }}">{{ __( 'Company' ) }}</a></p>
								</div>
							</div>
						</div>
					</div>
		...
		<script src="{{ asset('vendor/shop/themes/default/pius.js?v=' . config( 'shop.version', 1 ) ) }}"></script>
		@yield('pius_scripts')
	</body>
</html>
