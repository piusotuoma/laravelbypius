...
<html lang="{{ $locale }}" dir="{{ $localeDir }}">
	<head>
		...
		@if( config('app.debug') !== true )
			<meta http-equiv="Content-Security-Policy" content="default-src 'self' data: blob: https://cdnjs.cloudflare.com https://cdn.jsdelivr.net; {{ config( 'shop.csp.backend', 'style-src \'unsafe-inline\' \'self\' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net; script-src \'unsafe-eval\' \'self\' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net; connect-src \'self\' https://*.deepl.com https://api.openai.com; img-src \'self\' data: blob: https://cdnjs.cloudflare.com https://cdn.jsdelivr.net https://*.tile.openstreetmap.org ; frame-src https://www.youtube.com https://player.vimeo.com' ) }}">
		@endif

		<title>Pius enterprises administration page</title>
		...
		@endif
		<link rel="stylesheet" href="<?= airoute( 'pius_shop_jqadm_file', ['site' => $site, 'locale' => 'en', 'type' => 'css'] ); ?>">

		<style nonce="{{ app( 'pius.context' )->get( false )->nonce() }}">
...
<?= $content ?>
		...
		<script src="<?= airoute( 'pius_shop_jqadm_file', array( 'site' => $site, 'locale' => 'en', 'type' => 'js' ) ); ?>"></script>
	</body>
</html>
